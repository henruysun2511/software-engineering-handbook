...
- [8.1. Caching](#81-caching)
  - [8.1.1. Spring Cache Abstraction: `@Cacheable`, `@CacheEvict`, `@CachePut`](#811-spring-cache-abstraction-cacheable-cacheevict-cacheput)
  - [8.1.2. Tích hợp Redis làm cache](#812-tích-hợp-redis-làm-cache)
- [8.2. Xử lý bất đồng bộ & Message Queue](#82-xử-lý-bất-đồng-bộ-message-queue)
  - [8.2.1. `@Async` trong Spring](#821-async-trong-spring)
  - [8.2.2. Message Queue: RabbitMQ vs Kafka](#822-message-queue-rabbitmq-vs-kafka)
  - [8.2.3. Tích hợp Spring Boot với RabbitMQ](#823-tích-hợp-spring-boot-với-rabbitmq)
  - [8.2.4. Tích hợp Spring Boot với Kafka](#824-tích-hợp-spring-boot-với-kafka)
- [8.3. Microservices với Spring Cloud](#83-microservices-với-spring-cloud)
  - [8.3.1. Kiến trúc Microservices so với Monolithic](#831-kiến-trúc-microservices-so-với-monolithic)
  - [8.3.2. Service Discovery: Spring Cloud Netflix Eureka](#832-service-discovery-spring-cloud-netflix-eureka)
  - [8.3.3. OpenFeign — giao tiếp giữa các service](#833-openfeign-giao-tiếp-giữa-các-service)
  - [8.3.4. Spring Cloud Gateway — API Gateway](#834-spring-cloud-gateway-api-gateway)
  - [8.3.5. Config Server — quản lý cấu hình tập trung](#835-config-server-quản-lý-cấu-hình-tập-trung)
  - [8.3.6. Circuit Breaker: Resilience4j](#836-circuit-breaker-resilience4j)
  - [8.3.7. Distributed Tracing: Zipkin, Micrometer Tracing](#837-distributed-tracing-zipkin-micrometer-tracing)
  - [8.3.8. Event-Driven Architecture giữa các Microservices](#838-event-driven-architecture-giữa-các-microservices)
  - [8.3.9. Domain-Driven Design (DDD) — khái niệm cơ bản](#839-domain-driven-design-ddd-khái-niệm-cơ-bản)
- [8.4. Logging & Monitoring](#84-logging-monitoring)
  - [8.4.1. Logback/SLF4J — cấu hình log](#841-logbackslf4j-cấu-hình-log)
  - [8.4.2. Spring Boot Actuator](#842-spring-boot-actuator)
  - [8.4.3. Tích hợp Prometheus + Grafana](#843-tích-hợp-prometheus-grafana)
  - [8.4.4. Centralized Logging với ELK Stack](#844-centralized-logging-với-elk-stack)
- [8.5. Performance & Tối ưu](#85-performance-tối-ưu)
  - [8.5.1. N+1 Query Problem và cách khắc phục](#851-n1-query-problem-và-cách-khắc-phục)
  - [8.5.2. Connection Pool (HikariCP)](#852-connection-pool-hikaricp)
  - [8.5.3. Tối ưu JPA/Hibernate (tổng hợp)](#853-tối-ưu-jpahibernate-tổng-hợp)
  - [8.5.4. Batch Processing với Spring Batch](#854-batch-processing-với-spring-batch)
- [8.6. Scheduling](#86-scheduling)
  - [8.6.1. `@Scheduled`, Quartz Scheduler](#861-scheduled-quartz-scheduler)
- [8.7. WebSocket với Spring Boot](#87-websocket-với-spring-boot)
  - [8.7.1. Khái niệm và Setup](#871-khái-niệm-và-setup)
- [8.8. Webhook](#88-webhook)
  - [8.8.1. Khái niệm và thiết kế Webhook Receiver (nhận webhook từ bên thứ 3)](#881-khái-niệm-và-thiết-kế-webhook-receiver-nhận-webhook-từ-bên-thứ-3)
  - [8.8.2. Gửi Webhook cho hệ thống khác (Webhook Sender)](#882-gửi-webhook-cho-hệ-thống-khác-webhook-sender)
- [8.9. GraphQL với Spring Boot (tùy chọn)](#89-graphql-với-spring-boot-tùy-chọn)
- [8.10. Gọi API ngoài](#810-gọi-api-ngoài)
  - [8.10.1. RestTemplate vs WebClient vs RestClient](#8101-resttemplate-vs-webclient-vs-restclient)
  - [8.10.2. Setup và tích hợp `RestClient` vào dự án hiện có](#8102-setup-và-tích-hợp-restclient-vào-dự-án-hiện-có)
  - [8.10.3. Xử lý lỗi khi gọi API ngoài](#8103-xử-lý-lỗi-khi-gọi-api-ngoài)
  - [8.10.4. Logging request/response khi gọi API ngoài](#8104-logging-requestresponse-khi-gọi-api-ngoài)
- [8.11. AI Integration](#811-ai-integration)
  - [8.11.1. Khái niệm và Setup Spring AI](#8111-khái-niệm-và-setup-spring-ai)
  - [8.11.2. Tích hợp `ChatClient` vào Service nghiệp vụ hiện có](#8112-tích-hợp-chatclient-vào-service-nghiệp-vụ-hiện-có)
  - [8.11.3. Structured Output — ép kết quả trả về đúng kiểu Java](#8113-structured-output-ép-kết-quả-trả-về-đúng-kiểu-java)
  - [8.11.4. Function Calling / Tools — cho phép AI gọi lại method Java của bạn](#8114-function-calling-tools-cho-phép-ai-gọi-lại-method-java-của-bạn)
  - [8.11.5. So sánh: Khi nào dùng AI Integration, khi nào không](#8115-so-sánh-khi-nào-dùng-ai-integration-khi-nào-không)
- [So sánh tổng hợp Chương 8](#so-sánh-tổng-hợp-chương-8)
- [Best Practices tổng hợp](#best-practices-tổng-hợp)
- [Anti-patterns tổng hợp](#anti-patterns-tổng-hợp)
- [Bài tập](#bài-tập)
- [Tổng kết](#tổng-kết)
